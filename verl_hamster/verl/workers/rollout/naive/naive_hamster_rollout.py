...
from ..base import BaseRollout, NoOpRolloutMixin
from verl.workers.rollout.naive.naive_rollout import NaiveRollout

__all__ = ["NaiveHamsterRollout"]

class NaiveHamsterRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        # print(
        #     "logits", logits.shape, logits.device, logits.dtype, 
        #     "labels", response.shape, response.device, response.dtype
        # )
    ...
    def update_weights(self, state_dict: Optional[dict] = None, *args: Any, **kwargs: Any):
        # 若完全不用，直接返回 None；若可能被误调用，也可尝试加载
        return None
